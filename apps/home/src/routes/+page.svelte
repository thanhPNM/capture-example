<script lang="ts">
    ...
    onMount(() => {
        ...
        if (cachedSessions) {
            const parsedCachedSessions = cachedSessions
                ? JSON.parse(cachedSessions)
                : []
        ...
        <div class="sessions-list" transition:slide|local>
            <ul>
                {#each $sessions as session, i}
                    <li
                        class={session.active ? 'active' : 'inactive'}
                        transition:slide|local
                    >
                        <a href={`/${session.id}`} target="_blank"
                            >{i + 1}. {session.id}</a
                        >
                        {#if session.active}
                            <Button
                                className="custom-button"
                                on:click={() => inactiveSession(session.id)}
                            >
                                <DeleteIcon width={20} />
                            </Button>
                        {/if}
